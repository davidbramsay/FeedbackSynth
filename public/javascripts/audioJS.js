		...
		$(this).rangeslider({
			...
			onSlide:function( pos, val){
				...
				if (typeof idAudio !== 'undefined'){
                	...
                	//update MidiState variable
                	if (midiLoopState == 2 && !modFlag){ 
                		volState = [];
                		console.log("CALLED");
						$("audio").each(function (e) {
						volState.push($(this).get(0).volume);	
						});
					}	
				}else {
...
function modWheel(value){
	...
	if (midiLoopState==2){
		...
		var ind = 0;
		var min = Math.min.apply(Math, volState);
		$("audio").each(function (e) {
			//if ($(this).attr('id')!= "you"){
				var volume = volState[ind++]*(1/127.0)*value*(1/min);
				volume = (volume > 1) ? 1 : volume;
				$(this).prop("volume",volume);
				modFlag = true;
				$(this).parent().find('input').val(volume).change();
				modFlag = false;
