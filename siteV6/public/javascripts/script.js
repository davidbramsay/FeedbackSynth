...
synthMixer = context.createGain ? context.createGain() : context.createGainNode();
synthMixer.gain.value = 1;
...
function addToChat(msg, username, color) {
  ...
  messages.innerHTML = username + '>> ' + msg + '<br>' + messages.innerHTML;
...
var websocketChat = {
  send: function(message) {
    rtc._socket.send(rtc._socket.id);
...
var dataChannelChat = {
  ...
  recv: function(channel, message, id) {
...
function initChat() {
  ...
  rtc.on(chat.event, function() {
    ...
    addToChat(data.messages, data.name, data.color.toString(16));
    changeInputDivName(data.name, data.id);
  });
}

function changeInputDivName(name, id){
//change DIV of person to match their username
  $("#remote" + id).siblings(".labelAudio").html("<font size=3em>"+name+"</font>");
}
...
function init() {
  ...
  userName = window.prompt("Please enter your name:", "1");
}
