  ...
  rtc.createStream = function(opt, onSuccess, onFail) {
    ...
    if (getUserMedia) {
      ...
      getUserMedia.call(navigator, options, function(stream) {
        ...
        //make a high shelf to cut out the high end
        var eqObject9 = context.createBiquadFilter();
        eqObject9.type = 'highshelf'; //other opts: lowpass,highpass,bandpass, lowshelf, highshelf,notch,allpass
        eqObject9.frequency.value = 2000;
        eqObject9.Q.value = 5;
        eqObject9.gain.value = -16;

        //make a bass boost to help it feed back in the bass
        var eqObject10 = context.createBiquadFilter();
        eqObject10.type = 'highshelf'; //other opts: lowpass,highpass,bandpass, lowshelf, highshelf,notch,allpass
        eqObject10.frequency.value = 180;
        eqObject10.Q.value = 1;
        eqObject10.gain.value = -15;


        eqObject[8].connect(eqObject9);
        eqObject9.connect(eqObject10);
        eqObject9.connect(analyser);

        delay = context.createDelay();
        delay.delayTime.value = 0;
        eqObject10.connect(delay);

        var processedStream = context.createMediaStreamDestination();
        delay.connect(processedStream);

        synthMixer.connect(eqObject[0]);

        rtc.streams.push(processedStream.stream);
        rtc.initializedStreams++;
        onSuccess(processedStream.stream);
