                ...
                if ($(event.target).closest(".loopEffect").attr("id") == "delayLoopEffect"){
                   ...
                   console.log('delay' + ' ' + this.cv);
					...
					if (gainKnobUpdate){
						...
						gainNode.gain.setValueAtTime(this.cv/9 + 1, now);
						//gainNode.gain.value = this.cv/9 + 1;
					} else {
						gainKnobValue = this.cv/9 + 1;
					}
...
function noteOn(noteNumber, velocity) {
	...
	if(!gainKnobUpdate && MIDILOOPTRIGGER){
		...
		var now = context.currentTime;
		gainNode.gain.cancelScheduledValues(now);
		gainNode.gain.setValueAtTime(gainKnobValue, now);
//		gainNode.gain.value = gainKnobValue;
		console.log('gainNode gain:' + gainKnobValue);
...
function noteOff(noteNumber) {
	...
	if (activeNotes.length == 0 && MIDILOOPTRIGGER){
		...
		console.log('gainNode gain 0, store:' + gainKnobValue);
